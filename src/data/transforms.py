...
# [修改1] 引入 tv_tensors 替代 datapoints
...
# =========================================================================
# [核心修复区] 手动定义兼容层，确保逻辑不简化，且能对接新 API
# =========================================================================

# 1. 修复 ToImageTensor -> 映射为 T.ToImage
@register
class ToImageTensor(T.ToImage):
    def __init__(self):
        super().__init__()

# 2. 修复 ConvertDtype -> 映射为 T.ToDtype
# [关键] 必须开启 scale=True，否则 int->float 不会缩放到 [0,1]，导致模型不收敛
@register
class ConvertDtype(T.ToDtype):
    def __init__(self, dtype=torch.float32, scale=True):
        super().__init__(dtype=dtype, scale=scale)

# 3. 修复 SanitizeBoundingBox -> 映射为 T.SanitizeBoundingBoxes (注意复数)
@register
...
class PadToSize(T.Pad):
    # [修改2] 更新类型检查列表，使用 tv_tensors 替代 datapoints
    _transformed_types = (
        Image.Image,
        tv_tensors.Image,
        tv_tensors.Video,
        tv_tensors.Mask,
        tv_tensors.BoundingBoxes, # [注意] 新版是复数
    )
...
class ConvertBox(T.Transform):
    _transformed_types = (
        tv_tensors.BoundingBoxes, # [修改3] 使用 tv_tensors
    )
    ...
    def _transform(self, inpt: Any, params: Dict[str, Any]) -> Any:
        if self.out_fmt:
            # [修改4] spatial_size 变更为 canvas_size
